...
// ── coordinate helpers (module-level for pure functions) ────────────────────
...
export function GameCanvas() {
  ...
  const pu = useRef<PowerUpManager>(new PowerUpManager(CW, CH))
  const fireballAngle = useRef(0)

  // ── init ambient particles + load videos ────────────────────────────────
  useEffect(() => {
  ...
  const physics = useCallback((dt: number) => {
    ...
    useGameStore.getState().updateTimer(dt)
    ...
    const baseMult  = Math.min(1 + st.hitCount * HIT_ACCEL, 3.8)
    const speedMult = baseMult * (1 + pm.speedMultiplierBonus)
    let bx = st.ballX + st.ballVX * speedMult
    let by = st.ballY + st.ballVY * speedMult
    ...
    // ── Paddle geometry ───────────────────────────────────────────────────
    const PH     = (BASE_PADDLE_H / CH) * settings.paddleSize
    ...
    const hitPaddle = (side: 'left' | 'right', py: number) => {
      ...
      const cy = toY(py)
      const relY = by - py
      const hitFrac    = relY / PH
      const bounceAngle = hitFrac * (Math.PI / 3.2)
      const spd = Math.sqrt(vx*vx + vy*vy) * 1.06
      ...
      // Fireball explosion on any hit while tier ≥ 3
      if (pm.leftTier >= 3 || pm.rightTier >= 3) {
        pm.triggerExplosion(cx, cy, 240)
        if (settings.screenShake) {
          fx.current.shakeX = (side === 'left' ? 1 : -1) * 14
          fx.current.shakeY = (Math.random() - 0.5) * 10
        }
      }

      // Newly unlocked tier — activate it
      if (pm.tierJustReached) {
        const { side: ts, tier } = pm.tierJustReached
        pm.activateTier(ts, tier, bx, by, vx, vy)
        // Tier-specific FX
        if (tier === 4 && settings.screenShake) {
          fx.current.bigShake = 1
        }
        audioEngine.playHit(st.combo + 1, tier)
      } else {
      ...
      // Sync power state to store for HUD
      useGameStore.getState().syncPowerState(
        pm.leftTier, pm.rightTier,
        pm.getStreak('left'), pm.getStreak('right'),
        pm.activeSide,
        pm.missile.active,
        pm.phantom.active,
        pm.tierJustReached,
    ...
    // ── Scoring ────────────────────────────────────────────────────────────
    const handleScore = (scoringPlayer: 'left' | 'right') => {
      const loser = scoringPlayer === 'left' ? 'right' : 'left'
      pm.onScore(scoringPlayer)   // resets loser's streak/tier
      useGameStore.getState().syncPowerState(
        pm.leftTier, pm.rightTier,
        pm.getStreak('left'), pm.getStreak('right'),
        pm.activeSide, pm.missile.active, pm.phantom.active, null,
      )
      useGameStore.getState().incrementScore(scoringPlayer)
      useGameStore.getState().addScorePopup(scoringPlayer)
      audioEngine.playScore()
      const side = scoringPlayer === 'left' ? 1 : -1
      fx.current.shakeX = side * -22; fx.current.shakeY = 18; fx.current.pulse = 0.55
      fx.current.flashColor = scoringPlayer === 'left' ? '#ff4422' : '#2244ff'
      fx.current.flashAlpha = 0.22
      useGameStore.getState().resetBall()
    }

    if (bx < -1.12) { handleScore('right'); return }
    if (bx >  1.12) { handleScore('left');  return }
    ...
    // ── Phantom ball update ────────────────────────────────────────────────
    if (pm.phantom.active) {
      const { scored } = pm.updatePhantom(ly, ry, PH, speedMult)
      if (scored) {
        useGameStore.getState().incrementScore(scored)
        useGameStore.getState().addScorePopup(scored)
      }
    }

    // ── Missile update ─────────────────────────────────────────────────────
    const targetY = pm.missile.firedBy === 'left' ? ry : ly
    const missileRes = pm.updateMissile(targetY, ly, ry, PH, toX, toY)
    if (missileRes.hit) {
      // Missile scored a point for the firing player
      const scorer = pm.missile.firedBy
      handleScore(scorer)
      // Big nuke shake
      if (settings.screenShake) {
        fx.current.bigShake = 1
        fx.current.shakeX = (scorer === 'left' ? -1 : 1) * 35
        fx.current.shakeY = 28
      }
      return
    }

    // ── Explosion overlay update ───────────────────────────────────────────
    pm.updateExplosions()

    // ── Fireball angle ─────────────────────────────────────────────────────
    fireballAngle.current += 0.035
    ...
    if (st2.finalSeconds) {
      ...
      const prevBeat = Math.floor(st2.timeLeft + dt)
  ...
  const render = useCallback((ts: number) => {
    ...
    // ── decay FX ──────────────────────────────────────────────────────────
    const f = fx.current
    // Nuke shake decays slower for drama
    if (f.bigShake > 0) {
      f.bigShake -= 0.03
      f.shakeX = Math.sin(ts * 0.08) * 28 * f.bigShake
      f.shakeY = Math.cos(ts * 0.11) * 22 * f.bigShake
    } else {
    ...
    // Side arena glow — intensify when power tiers active
    const lIntensity = pm.leftTier  >= 3 ? 0.18 : pm.leftTier  >= 1 ? 0.10 : 0.07
    const rIntensity = pm.rightTier >= 3 ? 0.18 : pm.rightTier >= 1 ? 0.10 : 0.07
    const lg = ctx.createLinearGradient(0,0,220,0)
    lg.addColorStop(0,`rgba(255,60,20,${lIntensity})`); lg.addColorStop(1,'rgba(255,60,20,0)')
    ctx.fillStyle = lg; ctx.fillRect(0,0,220,CH)
    const rgl = ctx.createLinearGradient(CW,0,CW-220,0)
    rgl.addColorStop(0,`rgba(20,80,255,${rIntensity})`); rgl.addColorStop(1,'rgba(20,80,255,0)')
    ...
    // ── Matrix rain (layer 1 — behind everything) ─────────────────────────
    if (pm.matrixColsLeft.length > 0 || pm.matrixColsRight.length > 0) {
      pm.drawMatrix(ctx)
    }
    ...
    ambPts.current.forEach(p => {
      ...
      ctx.fillStyle = p.color; ctx.shadowBlur = 3; ctx.shadowColor = p.color
      ctx.fill()
    })
    ...
    const trailColor = pm.activeTier >= 3 ? ['#ff8800','#ffcc00'] :
                       pm.activeTier >= 1 ? ['#00ff44','#88ff88'] : tc
    trail.current.forEach((p,i)=>{
      ...
      ctx.fillStyle = trailColor[i%trailColor.length]
      ctx.shadowBlur=6; ctx.shadowColor=trailColor[i%trailColor.length]; ctx.fill()
    })
    ctx.globalAlpha=1; ctx.shadowBlur=0
    ...
    if (st.settings.particles) {
      st.particles.forEach(p=>{
        ...
        ctx.fillStyle = p.color; ctx.shadowBlur=5; ctx.shadowColor=p.color; ctx.fill()
      })
      ctx.globalAlpha=1; ctx.shadowBlur=0
    }

    // ── Phantom ball ───────────────────────────────────────────────────────
    pm.drawPhantom(ctx, toX, toY)

    // ── Missile + trail ────────────────────────────────────────────────────
    pm.drawMissile(ctx, toX, toY)

    // ── Paddles ────────────────────────────────────────────────────────────
    const paddleH = BASE_PADDLE_H * st.settings.paddleSize
    // Paddle tints when their tier is active
    const lCol1 = pm.leftTier >= 3  ? '#ff6600' : pm.leftTier  >= 1 ? '#44ff88' : '#ff3300'
    const lCol2 = pm.leftTier >= 3  ? '#ffaa44' : pm.leftTier  >= 1 ? '#88ffaa' : '#ff7744'
    const rCol1 = pm.rightTier >= 3 ? '#ff6600' : pm.rightTier >= 1 ? '#4488ff' : '#0044ff'
    const rCol2 = pm.rightTier >= 3 ? '#ffaa44' : pm.rightTier >= 1 ? '#88aaff' : '#4488ff'
    drawPaddle(ctx, toX(-0.895), toY(st.leftPaddleY),  PADDLE_W, paddleH, lCol1, lCol2, f.paddleStretchL, st.leftFaceDetected,  st.lastHitPlayer==='left')
    drawPaddle(ctx, toX( 0.895), toY(st.rightPaddleY), PADDLE_W, paddleH, rCol1, rCol2, f.paddleStretchR, st.rightFaceDetected, st.lastHitPlayer==='right')

    // ── Ball ───────────────────────────────────────────────────────────────
    const bCX = toX(st.ballX), bCY = toY(st.ballY)
    const bc1 = pm.activeTier >= 3 ? '#ff8800' : tc[0]
    const bc2 = pm.activeTier >= 3 ? '#ffee00' : (tc[1]||'#ffffff')

    // Motion streak
    const bspdV = Math.sqrt(st.ballVX*st.ballVX+st.ballVY*st.ballVY)
    if (bspdV > 0.012 && st.phase==='playing') {
      const sl = Math.min(55, bspdV * 2200)
    ...
    // If fireball active, draw video overlay instead of glow halos
    if (pm.activeTier >= 3 && pm.fireballVideo) {
      pm.drawFireball(ctx, bCX, bCY, fireballAngle.current)
    } else {
      // Normal glow halos
      for(let i=4;i>=0;i--){
    ...
    // Core ball (always drawn — visible through fireball if video fails)
    ...
    ctx.fillStyle=bg
    ctx.shadowBlur = pm.activeTier >= 3 ? 40 : 28+st.speedTier*8
    ctx.shadowColor=bc1; ctx.fill(); ctx.shadowBlur=0
    // Specular
    ctx.beginPath(); ctx.arc(bCX-4,bCY-4,4,0,Math.PI*2)
    ctx.fillStyle='rgba(255,255,255,0.55)'; ctx.fill()

    // ── Explosion overlays (drawn above ball, below HUD) ──────────────────
    pm.drawExplosions(ctx)
  ...
  function drawWall(ctx: CanvasRenderingContext2D, x:number,y:number,w:number,h:number,pos:'top'|'bottom',t:number){
    ...
    const lgw=ctx.createLinearGradient(0,ly,w,ly)
    lgw.addColorStop(0,'rgba(100,160,255,0)'); lgw.addColorStop(0.1,'rgba(150,200,255,0.9)')
    lgw.addColorStop(0.5,'rgba(220,240,255,1)'); lgw.addColorStop(0.9,'rgba(150,200,255,0.9)')
    lgw.addColorStop(1,'rgba(100,160,255,0)')
    ctx.strokeStyle=lgw; ctx.lineWidth=2; ctx.shadowBlur=11; ctx.shadowColor='#88aaff'
  ...
  const loop = useCallback((ts: number) => {
    ...
    lastT.current = ts
    const st = useGameStore.getState()
    // Reset power manager when not playing
